...
/**
 * Mirror of `app/ui/theme/TerminalThemes.kt`. The 9 presets are data-only,
 * so they could live in `:shared`; for Phase 2.x we duplicate to stay strictly
 * additive on the Desktop side (zero touch on `:shared` or `:app`).
 */
enum class TerminalThemeId(val displayName: String) {
    ...
    companion object {
        fun fromNameOrDefault(name: String): TerminalThemeId =
            entries.firstOrNull { it.name == name } ?: TECHTICAL_DARK
    }
}
...
fun paletteFor(themeId: TerminalThemeId): TerminalThemePalette = TERMINAL_THEMES.getValue(themeId)

/**
 * A theme as carried through the terminal session: the raw [name] stored on the
 * host ([TerminalThemeId] enum NAME for a preset, or a custom theme UUID) plus the
 * resolved [palette] used for rendering. Bundling both keeps the picker selection
 * (by name) and the renderer (by palette) consistent without re-resolving downstream.
 */
data class ResolvedTheme(
    val name: String,
    val palette: TerminalThemePalette,
)
...
/**
 * Resolves a host's `terminalTheme` value to a [ResolvedTheme] (decision 2):
...
fun resolveTheme(
    ...
    if (preset != null) return ResolvedTheme(preset.name, TERMINAL_THEMES.getValue(preset))
    val custom = customThemes.firstOrNull { it.id == themeName }
    if (custom != null) return ResolvedTheme(custom.id, custom.toPalette())
    return ResolvedTheme(
        TerminalThemeId.TECHTICAL_DARK.name,
        TERMINAL_THEMES.getValue(TerminalThemeId.TECHTICAL_DARK),
    )
}

/** Default [ResolvedTheme] for newly created sessions before any host resolution. */
fun defaultResolvedTheme(): ResolvedTheme = ResolvedTheme(
    TerminalThemeId.TECHTICAL_DARK.name,
    TERMINAL_THEMES.getValue(TerminalThemeId.TECHTICAL_DARK),
)
